...
fun OverviewView(
    ...
    LazyColumn(
        ...
        item {
            Row(Modifier.fillMaxWidth(), Arrangement.spacedBy(12.dp)) {
                listOf(
                    "活跃连接" to "${connections.size}",
                    "规则命中" to "${topRules.sumOf { it.optJSONObject("extra")?.optInt("hitCount", 0) ?: 0 }}"
                ).forEach { (label, value) ->
                    Card(
                        Modifier.weight(1f),
        ...
        // 高频规则命中
        item {
            Text("高频规则命中", fontWeight = FontWeight.Black, style = MaterialTheme.typography.titleSmall, modifier = Modifier.padding(top = 8.dp))
        }
        items(topRules) { rule ->
    ...
    LazyColumn(
        ...
        item {
            Row(
                ...
                IconButton(onClick = {
                    scope.launch { ApiClient.deleteAllConnections() }
                }) {
            ...
            Card(shape = RoundedCornerShape(16.dp), modifier = Modifier.fillMaxWidth().fillMaxHeight(0.7f)) {
                Column(Modifier.padding(16.dp)) {
                    ...
                    Box(Modifier.weight(1f).verticalScroll(rememberScrollState())) {
                        SelectionContainer { Text(selectedJson!!, fontSize = 11.sp, color = MaterialTheme.colorScheme.secondary) }
                    }
...
fun LogsView(
    ...
    Column(Modifier.fillMaxSize()) {
        Row(
            Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 8.dp).height(40.dp),
            verticalAlignment = Alignment.CenterVertically
        ) {
